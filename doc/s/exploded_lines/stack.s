...
.if module.included == 0
  ...
  .macro stack,  self,  varg:vararg
    ...
    .if obj.ndef
      ...
      stack.mut \self
    .else;
      mut.call \self, reset, default, stack
    .endif;
  ...
  .macro stack.__rept,  self,  step,  start,  end,  macro,  va:vararg
    ...
    .else;
      sidx.__rept \self, \step, \start, \end, "\macro", \va
    .endif;
  .endm;
  ...
  .macro stack.__fill,  self,  start,  size,  fill
    ...
    .endif;
  .endm;
  .macro stack.mut.fill.default,  va:vararg
    .altmacro
    stack.__fill \va
  .endm;
  .macro stack.mut.s.default,  self,  idx,  va:vararg
    .ifb \idx
      stack.mut.s.default \self, \self\().ss
    .else;
      \self\().s=\idx
      .if \idx > \self\().ss
        \self\().s=\self\().ss
    ...
    .endif;
  .endm;
  .macro stack.mut.ss.default,  self,  idx,  va:vararg
    .ifb \idx
      stack.mut.ss.default \self, \self\().ss
    .else;
      \self\().s=\idx
      .if \idx > \self\().sss
        \self\().s = \self\().sss
    ...
    .endif;
  .endm;
  .macro stack.mut.q.default,  self,  idx,  va:vararg
    .ifb \idx
      stack.mut.q.default \self, \self\().qq
    .else;
      \self\().q=\idx
      .if \idx < \self\().qq
        \self\().q=\self\().qq
    ...
    .endif;
  .endm;
  .macro stack.mut.new.default,  self,  size=1,  fill,  va:vararg
    .ifb \fill
      stack.mut.new.default \self, \size, \self\().fill
    .else;
      stack.fill \self, \self\().ss+1, \size, \fill
    .endif;
  .endm;
  .macro stack.mut.popm.default,  self,  va:vararg
    stack.pop \self
    .ifnb \va
      stack.pop \self, \va
    .endif;
  .endm;
  .macro stack.mut.reset.default,  self,  fill,  start,  size
    .ifb \start
      stack.mut.reset.default \self, \fill, \self\().qq, \size
      .exitm
    .endif;
    .ifb \size
      stack.mut.reset.default \self, \fill, \start, \self\().init_size
      .exitm
    .endif;
    \self\().q = \start
    \self\().s = \self\().q + \size
    ...
    .endif;
  .endm;
  .macro stack.mut.push.default,  self,  va:vararg
    .ifb \va
      stack.mut.push.default, \self, \self
    .else;
      ifalt
      stack.memalt = alt
      .altmacro
      .irp val,  \va
        .ifnb \val
          .if \self\().s < \self\().sss
            .if \self\().s >= \self\().ss
              stack.oob=1
              mut.call \self, oob_push, incr, stack, , , \val
            .endif;
            .if stack.oob==0
    ...
    .endif;
  .endm;
  .macro stack.mut.pop.default,  self,  va:vararg
    .ifb \va
      stack.mut.pop.default \self, \self\().pop
    .else;
      ifalt
      stack.memalt = alt
      .altmacro
      .irp sym,  \va
        .ifnb \sym
          .if \self\().s <= \self\().q
            stack.oob=1
            mut.call \self, oob_pop, null, stack, , , \sym
          .endif;
          .if stack.oob==0
            \self\().pop = \self
            \sym = \self
    ...
    .endif;
  .endm;
  .macro stack.mut.deq.default,  self,  va:vararg
    .ifb \va
      stack.mut.deq.default, \self, \self\().deq
    .else;
      stack.memalt = alt
      .altmacro
      .irp sym,  \va
        .ifnb \sym
          ...
          .if \self\().q+1 >= \self\().s
            stack.oob=1
            mut.call \self, oob_deq, null, stack, , , \sym
          .endif;
          .if stack.oob==0
  ...
  .macro stack.mut.oob_pop.default,  va:vararg
    stack.mut.oob_pop.null \va
  .endm;
  .macro stack.mut.oob_deq.default,  va:vararg
    stack.mut.oob_deq.null \va
  .endm;
  .macro stack.mut.oob_push.default,  va:vararg
    stack.mut.oob_push.incr \va
  .endm;
