...
.if module.included == 0
  ...
  .macro items.method,  self,  __ppt
    ...
    .if \self\().is_items_method == 0
      items.alloc \__ppt
      \self\().is_items_method = \__ppt
      .macro \self
      .endm;
    .endif;
    .purgem \self
    .macro \self,  va:vararg
      items.mut.method.default \self, \__ppt, \va
    .endm;
    items.__altm_reset
  .endm;
  .macro items.mut.method.default,  self,  ppt,  m,  va:vararg
    .if \ppt >= 0
      .ifb \m\va
        items.mut.blank.default \self, \ppt
      .elseif \ppt==0;
        items.mut.null.default \self, \ppt
      .endif;
      .ifb \m
        items.append \ppt, \va
      .else;
    ...
    .endif;
  .endm;
  .macro items.mut.blank.default,  self,  ppt
    .if \self\().is_items_method == \ppt
      items.free \self\().is_items_method
      \ppt = 0
    .else;
      \ppt = \self\().is_items_method
    .endif;
  .endm;
  .macro items.mut.null.default,  self,  ppt
    items.free \self\().is_items_method
    items.alloc \ppt
    \self\().is_items_method = \ppt
  .endm;
  .macro items.alloc,  __va:vararg=items
    items.__altm
    .irp __obj,  \__va
      .if items.__free$ <= 0
        items.__new
    ...
    items.__altm_reset
  .endm;
  .macro items.free,  __va:vararg=items
    items.__altm
    .irp __pntr,  \__va
      .ifnb \__pntr
        .if (\__pntr > 0)
    ...
    items.__altm_reset
  .endm;
  .macro items.append,  __pntr=items,  __va:vararg
    .if \__pntr > 0
      items.__altm
      .ifb \__va
  ...
  .macro items.emit,  __pntr=items,  __mcro=items.__statement,  __va:vararg
    ...
    .endif;
  .endm;
  .macro items.irp,  __pntr=items,  __mcro,  __va:vararg
    .if \__pntr > 0
      items.__altm
      items.__em <items.__mem>, %\__pntr, <.buf items.__irp, \__mcro, \__va>
    .endif;
  .endm;
  .macro items.irpc,  __pntr=items,  __mcro,  __va:vararg
    .if \__pntr > 0
      items.__altm
      items.__em <items.__mem>, %\__pntr, <.buf items.__irpc, \__mcro, \__va>
    .endif;
  .endm;
  .macro items.__irp,  __mcro,  __va:vararg
    .irp __itm,  \__va
      .ifnb \__itm
        \__mcro \__itm
      .endif;
    .endr;
  .endm;
  .macro items.__irpc,  __mcro,  __va:vararg
    .irpc __itm,  \__va
      .ifnb \__itm
        \__mcro \__itm
      .endif;
    .endr;
  .endm;
  .macro items.__statement,  m,  va:vararg
    \m \va
  .endm;
  .macro items.__new
  ...
  .endm;
  .macro items.__alloc,  __obj,  __pntr
    \__obj = \__pntr
    items.__em <items.__build items.__mem>, %\__pntr
    items.__free$ = items.__free$ - 1
    items.__update
  .endm;
  .macro items.__build,  __self
    .if \__self\().purgem
      .purgem \__self\().buf
    .endif;
    \__self\().purgem = 1
    .noaltmacro
    .macro \__self\().buf,  m,  arg,  va:vararg
      items.__altm_reset
      \m \arg \va
    .endm;
    .altmacro
  .endm;
  .macro items.__free,  __self
    .if \__self\().purgem
      .purgem \__self\().buf
      \__self\().purgem = 0
      items.__free$ = items.__free$ + 1
    ...
    .endif;
  .endm;
  .macro items.__append,  __self,  __varg:vararg
    ...
    .altmacro
  .endm;
  .macro items.__update
    items.free = items.__free$
    items.alloc = items.__mem$ - items.__free$
  .endm;
  .macro items.__em,  p,  i,  s,  va:vararg
    \p\()$\i\s\va
  .endm;
  .macro items.__em2,  p,  i,  s,  i2,  s2,  va:vararg
    \p\()$\i\s\()$\i2\s2\va
  .endm;
  .macro items.__altm
    .irp _altm,  %1
