...
.if module.included == 0
  ...
  .macro list,  self,  varg:vararg
    ...
    .if \self\().is_list == 0
      ...
      .endm;
      .macro \self\().new,  size=1,  fill=\self\().fill,  va:vararg
  ...
  .endm;
  .macro stack.mut.get.default,  self,  idx,  sym,  va:vararg
    .ifb \sym\va
      stack.mut.get.default \self, \idx, \self
    .else;
      .ifb \idx
        \self\().i = \self\().i - 1
      .else;
    ...
    .endif;
  .endm;
  .macro stack.mut.set.default,  self,  idx,  val,  va:vararg
    \self\().i = \idx - 1
    .irp x,  \val,  \va
      \self\().i = \self\().i + 1
      .ifnb \x
    ...
    .endr;
  .endm;
  .macro stack.mut.iter.default,  self,  va:vararg
    .ifb \va
      stack.mut.iter.default \self, \self
    .else;
      .irp sym,  \va
        .ifnb \sym
          ...
          .endif;
          .if stack.oob
            mut.call \self, oob_iter, rot, stack, , , \va
          .endif;
          .if stack.oob==0
            \self\().i = stack.idx
            \self\().iter = \self
