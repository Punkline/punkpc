...
.if module.included == 0
  ...
  .endm;
  .macro sidx.__rept,  self,  step,  start=0,  end=0,  macro,  va:vararg
    .if (\step > 0) && (\start > \end)
      sidx.__rept \self, -1, \start, \end, "\macro", \va
    .else;
      sidx.__rept = \start
      sidx.__rept_count = (\end+1) - \start
      .if sidx.__rept_count < 0
        sidx.__rept_count = -sidx.__rept_count +2
      .endif;
      .if sidx.__rept_count
        .rept sidx.__rept_count
          .ifb \va
            sidx.noalt "<sidx.__rept_iter \macro, \self>", sidx.__rept
          .else;
            sidx.noalt "<sidx.__rept_iter \macro, \self>", sidx.__rept, , , \va
          .endif;
          sidx.__rept = sidx.__rept + \step
        .endr;
        ifalt.reset sidx.alt
        alt = sidx.memalt
      .endif;
    .endif;
  .endm;
  .macro sidx.__rept_iter,  macro,  mem,  va:vararg
    ifalt.reset sidx.alt
    \macro \mem \va
  .endm;
.endif;
