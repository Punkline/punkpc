...
.if module.included == 0
  ...
  .macro enum.mut.restart.sp_obj,  self,  va:vararg
    ...
    \self\().__has_items = 0
  .endm;
  .macro enum.mut.enum_parse.sp.temp,  va:vararg
    enum.mut.enum_parse.default \va
    .if sp.temp.bytes > 0
      sp.temp.__has_items = 1
    .endif;
  .endm;
  .macro enum.mut.enum_parse.sp.gprs,  va:vararg
    .ifnb \va
      enum.mut.enum_parse.default \va
      .if sp.gprs.__has_items == 0
        .if sp.gprs.bytes
          sp.gprs.__has_items = 1
          sidx.noalt2 "<stmw sp.gprs.lowest>", sp.mem_ID, "<, sp.gprs.base>", sp.mem_ID, "<(sp)>"
        .endif;
      .endif;
    .endif;
  .endm;
  .macro enum.mut.enum_parse.sp.fprs,  va:vararg
    .ifnb \va
      enum.mut.enum_parse.default \va
    .endif;
    ...
    .endif;
  .endm;
  .macro enum.mut.enum_parse_iter.sp.sprs,  self,  spr,  va:vararg
  ...
  .macro sp.push,  va:vararg
    ...
    .irp arg,  \va
      .ifnb \arg
        ...
        num = sp.chars$0
        .if num == 'r
          sp.__checkreg \arg, sp.gprs
        .elseif num == 'f;
          sp.__checkreg \arg, sp.fprs
        .elseif num == 'x;
          sp.__checkx \arg, sp.temp.__items
        .else;
          sp.__checkelse \arg
        .endif;
      .endif;
    ...
    .if sp.lr.__has_items
      mflr r0
    .endif;
    sidx.noalt "<stwu sp, -sp.frame>", sp.mem_ID, "<(sp)>"
    .if sp.lr.__has_items
      sidx.noalt "<stw r0, sp.frame>", sp.mem_ID, "< + 4 (sp)>"
    .endif;
  ...
  .macro sp.commit
    ...
    .endif;
  .endm;
  .macro sp.pop,  va:vararg
    regs.restart
    sp.commit
    .if sp.fprs.__has_items
      lmfd 31, sp.fprs.base(sp), sp.fprs.lowest
    .endif;
    .if sp.gprs.__has_items
      lmw sp.gprs.lowest, sp.gprs.base(sp)
    .endif;
    .if sp.sprs.__has_items
      sp.sprs.__items sp.__lmspr
    .endif;
    .if sp.lr.__has_items
      lwz r0, sp.frame+4(sp)
    .endif;
    addi sp, sp, sp.frame
    .if sp.lr.__has_items
      mtlr r0
