...
.ifeq stack.included;  stack.included = 0x100
  ...
  stack$ = 0;stack.idx = 0;stack.oob = 0;mut.class stack
  .macro stack,  self,  varg:vararg;  ifdef \self\().isStack
    .if ndef;  stack$=stack$+1;\self\().isStack=stack$;\self\().s = size;ifdef \self
      ...
      \self\().fill = stack.fill;\self\().step = 1;\self\().init = stack.size
      \self\().sss = stack.sss
      .if \self\().init > 0
      ...
      .endm;.macro \self\().get,  idx=\self\().i,  sym=\self,  va:vararg
        ...
        .irp x,  \sym,  \va;  \self\().i = \self\().i + 1
          .ifnb \x;  sidx.get \self, \self\().i;\x = sidx
            \self\().hook.get_plugin \self, \x;.endif;.endr;
      .endm;.macro \self\().set,  idx=\self\().i,  val=\self,  va:vararg
        \self\().i = \idx - 1
        .irp x,  \val,  \va;  \self\().i = \self\().i + 1
          .ifnb \x;  sidx = \x;sidx.set \self, \self\().i
            \self\().hook.set_plugin \self, \x;.endif;.endr;
      ...
      .endm;.macro \self\().popm,  va:vararg;  \self\().pop
      ...
      .endm;stack.mut \self;\self\().push.mode incr;\self\().pop.mode null
      \self\().deq.mode null;\self\().iter.mode rot;\self\().i.mode rot, range
      \self\().mut , i_plugin, get_plugin, set_plugin, iter_plugin, push_pre, push_post;.endif;
  ...
  .endm;.macro stack.fill,  self,  start,  size,  fill;  LOCAL i, idx
    ...
    .if (idx <= \self\().ss) && (\self\().ss < (idx+i))
      \self\().ss = idx+i;.endif;
    .if i > 0
      .rept i;  sidx.ema \self, %idx, <=\fill>
        idx=idx+1;.endr;.endif;
  .endm;.macro stack.reset,  self,  fill,  start,  size;  \self\().q = \start
    \self\().s = \self\().q + \size
  ...
  .endm;.macro stack.push,  self,  va:vararg;  ifalt;stack.memalt = alt;.altmacro
    .irp val,  \va
      .ifnb \val;  \self\().hook.push_pre \self, \val
        .if \self\().s < \self\().sss
          .if \self\().s >= \self\().ss
            stack.oob=1;\self\().hook.oob_push \self, \val;.endif;
          .if stack.oob==0;  sidx.ema \self, %\self\().s, <=\val>
            \self\().s = \self\().s + 1;\self = \self\().fill
            \self\().hook.push_post \self, \val;.endif;.endif;.endif;stack.oob=0;.endr;
    ifalt.reset stack.memalt;stack.oob=0
  .endm;.macro stack.pop,  self,  va:vararg;  ifalt;stack.memalt = alt;.altmacro
    .irp sym,  \va
      .ifnb \sym
        .if \self\().s <= \self\().q
          stack.oob=1;\self\().hook.oob_pop \self, \sym;.endif;
        .if stack.oob==0;  \self\().pop = \self;\sym = \self;\self\().s = \self\().s - 1
          sidx.ema <\self = \self>, %\self\().s
        .endif;.endif;stack.oob=0;.endr;ifalt.reset stack.memalt;stack.oob=0
    ...
    .irp sym,  \va
      .ifnb \sym;  stack.idx = \self\().i + \self\().step
        ...
        .if stack.oob==0;  \self\().i = stack.idx;\self\().iter = \self;\sym = \self
          \self\().get;\self\().hook.iter_plugin \self, \sym;.endif;.endif;stack.oob=0;.endr;
    stack.oob=0
  ...
  .endm;.macro stack.mut.idx_i.abs,  self,  idx,  va:vararg;  \self\().hook.oob_i \self, \idx
