...
.ifeq str.included;  str.included=0x100
  ...
  str$=0;str.vacount=0;str.logic=0;str.force_litmem=0;str.self_pointers=1;str.show_warnings=1
  str.mRead = 1;str.mWrite=0;str.mLitmem = 2;str.mStrmem=0;str.mLitio = 4;str.mStrio=0
  str.mPrefix = 8;str.mSuffix=0;str.mAltstr = 16;str.mNoalt=0
  ...
  .endm;.macro str.vachars,  va:vararg;  str.vacount=0
    .irpc c,  \va;  str.vacount = str.vacount+1;.endr;
  ...
  .endm;.macro str,  self,  varg:vararg;  ifalt;ifalt;ifdef \self\().isStr
    ...
    .if \self\().isStr == 0;  str$ = str$ + 1;\self\().isStr = str$;\self\().litmode=0
      \self\().isBlankStr=1
      .if str.self_pointers;  \self = str$;.endif;.altmacro
      $_str.point$ \self, %\self\().isStr;ifalt.reset
  ...
  .endm;.macro str.strbuf_commasuf,  self,  cb,  a,  va:vararg;  str.vacount \va
    .if str.vacount
      ...
      .else;  \self\().strbuf_event \cb, <\a>
      .endif;.endif;
  .endm;.macro str.qrecurse_iter,  m,  str,  va:vararg;  \m "\str"
    .ifnb \va;  str.qrecurse_iter \m, \va;.endif;
  .endm;.macro str.qrecurse,  va:vararg;  ifalt
    .if alt;  str.qrecurse_alt \va
    .else;  str.qrecurse_nalt \va;.endif;
  ...
  .endm;.macro str.warnings,  va:vararg
    .if str.show_warnings;  str.qrecurse_iter .warning, \va;.endif;
  .endm;.macro str.warning,  va:vararg
    .if str.show_warnings;  str.qrecurse .warning, \va;.endif;
  ...
  .endm;.macro $_str.point$,  self,  id
    .macro $_str.point$\id,  m,  va:vararg
      ...
      .else;  \m \self;.endif;
    .endm;.endm;.macro str.point.get,  str;  ndef=0;def=1;ifalt
    .irpc c,  \str
      .irpc n,  0123456789+-*%/&^!~()[]
        .ifc \c,  \n;  def=0;.exitm;.endif;.endr;.exitm;.endr;
    .if def;  ifdef \str\().isStr;.endif;
    .if def;  str.point = \str\().isStr
    .else;  ifdef \str
      .if def;  def=0;ndef=1;str.point = \str
      .else;  str.point = 0;.endif;.endif;ifalt.reset
  .endm;.macro str.count.items,  str;  str.point.get \str;str.litq str.point, str.vacount
    str.count = str.vacount
  .endm;.macro str.count.chars,  str;  str.point.get \str;str.strq str.point, str.vachars
    str.count = str.vacount
  ...
  .endm;.macro str.irpc,  str,  va:vararg;  str.point.get \str
    str.point, str.irpc_handle, 0, \va
  .endm;.macro str.irpcq,  str,  va:vararg;  str.point.get \str
    str.point, str.irpc_handle, 1, \va
  .endm;.macro str.irpc_handle,  str,  q,  m,  va:vararg;  str str.irpc ".irpc char,"
    str.vacount \va
    .if str.vacount == 1
      .ifb \va;  str.vacount=0;.endif;.endif;
    .if str.vacount
  ...
  .endm;.macro str.strbuf_event$9,  self,  a,  str,  va:vararg;  \a \va "\str"
