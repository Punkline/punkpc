...
.if module.included == 0
  ...
  .macro enum.new_raw,  self,  prefix,  suffix,  varg:vararg
    ...
    .if ndef
      ...
      .endm;
      .macro \self\().enum_conc,  va:vararg
        ifalt
        enum.ifalt = alt
        .noaltmacro
      ...
      .endm;
      .macro \self\().restart,  va:vararg
        \self\().hook.restart \va
      .endm;
    .endif;
  .endm;
    ...
    .if ndef
      ...
      .macro \self\().mask_conc,  pfx,  sfx,  va:vararg
        .irp arg,  \va
          .ifnb \arg
            .irp conc,  m\arg\sfx
              \self\().hook.mask \pfx\arg\sfx, \pfx\conc
            .endr;
          .endif;
        .endr;
      .endm;
  ...
  .macro enum.mut.enum_parse_iter.default,  self,  sym,  pfx,  sfx,  arg,  va:vararg
    ifnum_ascii \arg
    .if num
      \self\().hook.numerical \arg, num, \pfx, \sfx, \va
    .else;
      \self\().hook.literal \sym, \pfx, \sfx, \arg, \va
    .endif;
  .endm;
  .macro enum.mut.numerical.default,  self,  arg,  char,  va:vararg
    .if \char == 0x28
      \self\().hook.count \arg, \va
    .elseif \char >= 0x30;
      .if \char <= 0x39
        \self\().hook.count \arg, \va
      .endif;
    .elseif \char == 0x2B;
      \self\().hook.step \arg, \va
    .elseif \char == 0x2D;
      \self\().hook.step \arg, \va
    .endif;
  .endm;
  ...
  .macro enum.mut.count.bool,  self,  arg,  va:vararg
    ...
    \self\().count = \arg & 0x1F
  .endm;
  .macro enum.mut.mask.bool,  self,  arg,  mask,  va:vararg
    ifnum \arg
    .if nnum
      ifdef \arg
      .if ndef
