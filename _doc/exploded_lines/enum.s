...
.if module.included == 0
  ...
  .macro enum.mut.enum_parse_iter.default,  self,  sym,  pfx,  sfx,  arg,  va:vararg
    ifnum_ascii \arg
    .if num
      mut.call \self, numerical, default, enum, , , \arg, num, \pfx, \sfx, \va
    .else;
      mut.call \self, literal, default, enum, , , \sym, \pfx, \sfx, \arg, \va
    .endif;
  .endm;
  .macro enum.mut.numerical.default,  self,  arg,  char,  va:vararg
    .if \char == 0x28
      mut.call \self, count, default, enum, , , \arg, \va
    .elseif \char >= 0x30;
      .if \char <= 0x39
        mut.call \self, count, default, enum, , , \arg, \va
      .endif;
    ...
    .endif;
  .endm;
  .macro enum.mut.literal.default,  self,  arg,  va:vararg
    \arg = \self\().count
    mut.call \self, count, default, enum, , , \self\().count + \self\().step
    \self\().steps = \self\().steps + 1
  .endm;
  .macro enum.mut.count.default,  self,  arg,  va:vararg
    \self\().last = \self\().count
    \self\().count = \arg
  .endm;
  .macro enum.mut.step.default,  self,  arg,  va:vararg
    \self\().step = \arg
  .endm;
  .macro enum.mut.restart.default,  self,  va:vararg
    \self\().step = \self\().step.restart
    \self\().count = \self\().count.restart
  .endm;
  .macro enum.mut.mask.default,  self,  pfx,  sfx,  va:vararg
    .irp arg,  \va
      .ifnb \arg
        .irp conc,  m\arg\sfx
          enum.__bool_mask \self, \pfx\arg\sfx, \pfx\conc
        .endr;
      .endif;
    .endr;
  .endm;
  .macro enum.__bool_mask,  self,  arg,  mask,  va:vararg
    ifnum \arg
    .if nnum
      ifdef \arg
      .if ndef
    ...
    .endif;
  .endm;
  .macro enum.mut.literal.bool,  self,  sym,  pfx,  sfx,  arg,  va:vararg
  ...
  .macro enum.enum_conc,  va:vararg
    ...
    .endr;
  .endm;
  .macro enum.restart,  self,  va:vararg
    .ifb \self
      .irp p,  restart.count,  restart.step
        enum.temp.\p = enum.\p
      .endr;
      enum.temp.restart
      .irp p,  count,  step
      ...
      .endr;
    .else;
      enum.call_mut \self, restart, default, \va
    .endif;
  .endm;
  ...
  .macro enumb.enum_conc,  va:vararg
    .irp p,  last,  count,  step,  steps
      enumb.temp.\p = enumb.\p
    .endr;
    enum.mut.enum_conc.default enumb.temp, \va
    .irp p,  last,  count,  step,  steps
      enumb.\p = enumb.temp.\p
    .endr;
  .endm;
  .macro enumb.restart
    .irp p,  count.restart,  step.restart
      enumb.temp.\p = enumb.\p
    .endr;
    enumb.temp.restart
    .irp p,  count,  step
