...
.if module.included == 0
  ...
  .macro obj.class,  class,  class_ppt,  dict=point,  get=pointer,  mut_ns=mut,  hook_ns=hook
    ...
    .if \class\().is_objClass == 0
      ...
      .irp param,  .uses_pointers,  .self_pointers,  .uses_mutators,  .uses_obj_mut_methods
        ...
        .endif;
        .irp conc,  .default
          obj.class\param = obj.class\param\conc
        .endr;
      .endr;
      ...
      .if \class\().uses_pointers
        ...
        .macro \class\().\get,  obj,  sym=\class\().\get
          ...
          .endif;
        .endm;
        .macro \class\().\dict,  point=\class\().\get,  va:vararg
          obj.vacount \point
          ifalt
          .altmacro
          .if obj.vacount > 1
          ...
          .endif;
        .endm;
        .macro \class\().\dict\()q,  point=\class\().\get,  va:vararg
          obj.vacount \point
          ifalt
          .altmacro
          .if obj.vacount > 1
          ...
          .endif;
        .endm;
        .macro \class\().call_method,  self=\class\().\get,  meth,  va:vararg
          \class\().\get \self
          \class\().\dict, obj.__call_method, \meth, \va
        .endm;
        .macro \class\().set_property,  self=\class\().\get,  ppt,  val=\class\().property
          \class\().\get \self
          \class\().\dict, obj.__set_property, \ppt, \val
        .endm;
        .macro \class\().get_property,  self=\class\().\get,  ppt,  sym=\class\().property
          \class\().\get \self
          \class\().\dict, obj.__get_property, \ppt, \sym
        .endm;
      .endif;
    .endif;
    ifalt.reset obj.state.alt
    alt = obj.state.altm
  .endm;
  .macro obj.vacount,  va:vararg
    obj.vacount=0
    .irp vacount,  \va
      obj.vacount=obj.vacount+1
    .endr;
  .endm;
  .macro obj.hidden_constructor,  obj,  constr,  va
    hidden "_", ", \obj, \va", \constr
  .endm;
  .macro obj.__call_method,  a,  b,  va:vararg
    \a\b \va
  .endm;
  .macro obj.__set_property,  a,  b,  c
    \a\b = \c
  .endm;
  .macro obj.__get_property,  a,  b,  c
    \c = \a\b
  .endm;
  .macro obj.__check_if_def,  obj,  ppt,  class,  dict
    ifdef \obj\ppt
    obj.def = def
    obj.ndef = ndef
    .if obj.ndef
    ...
    .endif;
  .endm;
  .macro obj.__def_obj_methods,  obj,  class,  mut_ns,  hook_ns,  varg:vararg
    .irp m,  \varg
      .ifnb \m
        \class\().purge_hook \obj, \m
    ...
    .endr;
  .endm;
  .macro obj.__def_class_methods,  class,  varg:vararg
    .irp m,  \varg
      .ifnb \m
        .macro \class\().\m,  obj,  va:vararg
          \class\().call_mut \obj, \m, default, \va
        .endm;
      .endif;
    .endr;
  .endm;
  .macro obj.class.dict.__recurse_start,  class,  dict,  point,  pointcheck,  va:vararg
  ...
  .macro obj.class.dict.__recurse_peak,  class,  dict,  stack,  point,  m,  va:vararg
    ...
    .ifb \va
      .if alt
        $.__\class\().\dict\()$\point <\m \stack>
      .else;
        $.__\class\().\dict\()$\point "\m \stack"
      .endif;
    .else;
      .if alt
        $.__\class\().\dict\()$\point <\m \stack, >, \va
      .else;
        $.__\class\().\dict\()$\point "\m \stack,", \va
      .endif;
    .endif;
  .endm;
  .macro obj.class.dict.__eval,  class,  dict,  point,  va:vararg
    ifalt.reset
    $.__\class\().\dict\()$\point \va
  .endm;
  .macro obj.class.dictq.__recurse_start,  class,  dict,  point,  pointcheck,  va:vararg
  ...
  .macro obj.class.dictq.__recurse,  obj,  class,  dict,  comma,  stack,  point,  pointcheck,  va:vararg
    ...
    .else;
      obj.class.dictq.__recurse_peak \class, \dict, <\stack\comma \obj>, \point, \va
    .endif;
  .endm;
  .macro obj.class.dictq.__recurse_peak,  class,  dict,  stack,  point,  m,  va:vararg
    ...
    .ifb \va
      .if alt
        $.__\class\().\dict\()q$\point <\m>, \stack
      .else;
        $.__\class\().\dict\()q$\point "\m", \stack
      .endif;
    .else;
      .if alt
        $.__\class\().\dict\()q$\point <\m>, \va, \stack
      .else;
        $.__\class\().\dict\()q$\point "\m", \va, \stack
      .endif;
    .endif;
  .endm;
  .macro obj.class.dictq.__eval,  class,  dict,  point,  va:vararg
    ifalt.reset
