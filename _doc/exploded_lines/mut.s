...
.ifeq mut.included
  ...
  .macro mut.class,  class,  mut_ns=mut,  hook_ns=hook
    ...
    .if ndef
      mut.mutableClass$ = mut.mutableClass$ + 1
    ...
    .endif;
  .endm;
  .macro mut.obj,  obj,  class,  mut_ns=mut,  hook_ns=hook
    ifdef \obj\().isMutableObj
    .if ndef
      mut.mutableObj$ = mut.mutableObj$ + 1
      \obj\().isMutableObj = mut.mutableObj$
  ...
  .macro mut.hook,  hook,  obj,  class,  mut_ns=mut,  hook_ns=hook
    ...
    .macro \obj\().\hook_ns\().\hook,  va:vararg
      \class\().\mut_ns\().\hook \va
    .endm;
    \obj\().\hook_ns\().\hook\().purgable = 1
  .endm;
  .macro mut.mut,  mut,  hook,  obj,  hook_ns=hook
    mut.purge \hook, \obj, \hook_ns
    .ifb \mut
      .macro \obj\().\hook_ns\().\hook,  va:vararg
      .endm;
    ...
    \obj\().\hook_ns\().\hook\().purgable = 1
  .endm;
  .macro mut.mode,  mode,  hook,  obj,  class,  mut_ns=mut,  hook_ns=hook
    mut.purge \hook, \obj, \hook_ns
    ifdef \class\().\mut_ns\().\hook\().mode$
    .if ndef
      \class\().\mut_ns\().\hook\().mode$ = 0
    .endif;
    ifdef \class\().\mut_ns\().\hook\().\mode\().isMutatorMode
    .if ndef
      mut.mutatorMode$ = mut.mutatorMode$ + 1
    ...
    .endif;
    .macro \obj\().\hook_ns\().\hook,  va:vararg
      \class\().\mut_ns\().\hook\().\mode \va
    .endm;
    \obj\().\hook_ns\().\hook\().mode = \class\().\mut_ns\().\hook\().\mode
    \obj\().\hook_ns\().\hook\().purgable = 1
  .endm;
  .macro mut.purge,  hook,  obj,  hook_ns=hook
    ifdef \obj\().\hook_ns\().\hook\().isMutator
    .if def
      .if \obj\().\hook_ns\().\hook\().purgable
        .purgem \obj\().\hook_ns\().\hook
      .endif;
    .else;
      mut.mutator$ = mut.mutator$ + 1
      \obj\().\hook_ns\().\hook\().isMutator = mut.mutator$
      \obj\().\hook_ns\().\hook\().mode = 0
      .macro \obj\().\hook_ns\().\hook\().mut,  mut
        mut.mut \mut, \hook, \obj, \hook_ns
      .endm;
      .macro \obj\().\hook_ns\().\hook\().mode,  mode
        \obj\().mode \hook, \mode
      .endm;
      .macro \obj\().\hook_ns\().\hook\().purge
        mut.purge \hook, \obj, \hook_ns
      .endm;
    .endif;
    \obj\().\hook_ns\().\hook\().mode = 0
